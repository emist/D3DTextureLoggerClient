...
{
	HMODULE WINAPI RemoteOps::GetRemoteModuleHandle(HANDLE hProcess, LPCSTR lpModuleName)
	{
		...
		for(DWORD i = 0; i <= NumModules; ++i)
		{
			...
			for (size_t i = 0; ModuleNameBuffer[i] != '\0'; ++i)
			{
				if (ModuleNameBuffer[i] >= 'A' && ModuleNameBuffer[i] <= 'Z')
					ModuleNameBuffer[i] += 0x20; // 0x20 is the difference between uppercase and lowercase
			}
	...
	FARPROC WINAPI RemoteOps::GetRemoteProcAddress (HANDLE hProcess, HMODULE hModule, LPCSTR lpProcName, UINT Ordinal, BOOL UseOrdinal)
	{
		BOOL Is64Bit = FALSE;
